...
summary(lin_model_read)$coefficients["z", ]
```
*An analysis utilizing Lin's estimator at the school level showed that the intervention is statistically significant for both ORF and reading comprehension. The contrast with previous results is likely due to the fact that the aggregated covariates at the school level weren't very meaningful and allowed for noise in the model. Also in this set up with have a higher DF given lower amount of covariates. 
