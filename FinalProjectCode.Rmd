...
X <- data[, c("grade", "female", "age", "language", "region", "school_type")]
Z <- data$treatment
...
num_control_schools <- sum(schools_treatment_counts$num_control > 0)
paste0("Number of control schools: ", num_control_schools)

num_treatment_schools <- sum(schools_treatment_counts$num_treatment > 0)
...
school_summary <- X %>%
  ...
  mutate(school_type = Mode(school_type)) %>%
...
* Now that we have propensity score and outcome model prediction fro each student/school, we can compute a doubly-robust AIPW estimand for each student. Our final estimand will be thge mean of these values.
